...
class HandRenderer
{
  public:
    HandRenderer(
      ...
      scorer(particles, 10.0, 20.0, 10, 40, size, size),
      tileSize(size),
      windowWidth(WINDOW_WIDTH),
      windowHeight(WINDOW_HEIGHT),
      // TODO get rid of one of these?
      renderWidth(sqrt(particles) * size),
      renderHeight(sqrt(particles) * size),
      numTiles(particles),
...
//      glEnable(GL_CULL_FACE);
      ...
      cv::Mat blank = cv::Mat::zeros(cv::Size(tileSize, tileSize), CV_8UC1);
      matToTexture(skinTexture, blank);
      blank = cv::Mat::zeros(cv::Size(tileSize, tileSize), CV_16UC1);
      ...
      displaySkin = cv::Mat(cv::Size(WINDOW_WIDTH, WINDOW_HEIGHT), CV_8UC1);
      return true;
    }
...
//        cv::waitKey(1000);
    ...
    bool run(string fileName)
    {
      ...
      // Copy for display in window
      skinImage.copyTo(displaySkin);
      ...
      cv::Mat depthHand = depthImage(cv::Rect(minX, minY, maxX - minX, maxY - minY));

      cv::resize(hand, skinImage, cv::Size(tileSize, tileSize));
      cv::resize(depthHand, depthImage, cv::Size(tileSize, tileSize));

      cv::Mat tempSkin;
      cv::flip(skinImage, tempSkin, 1);
//      imshow("skin", tempSkin);
...
//      cv::waitKey(30);
      ...
      // Now update the projection for rendering
      //
      // First translate to origin
     
      double midX = minX + double(maxX - minX)/2, midY = minY + double(maxY - minY)/2;

      cout << midX << " " << midY << endl;

      double transX, transY;
      cout << midX - WINDOW_WIDTH/2 << endl;
      cout << midY - WINDOW_HEIGHT/2 << endl;
      cout << -2 * ((midX - WINDOW_WIDTH/2) / WINDOW_WIDTH) << endl;
      cout << 2 * ((midY - WINDOW_HEIGHT/2) / WINDOW_HEIGHT) << endl;
      transX = 2 * ((midX - WINDOW_WIDTH/2) / WINDOW_WIDTH);
      transY = 2 * ((midY - WINDOW_HEIGHT/2) / WINDOW_HEIGHT);

      cout << "TransX: " << transX << " TransY: " << transY << endl;

      // Now scale
      double scaleX, scaleY;
      scaleX = WINDOW_WIDTH / double(maxX - minX);
      scaleY = WINDOW_HEIGHT / double(maxY - minY);
...
//      scaleY = 2;
      ...
      glm::mat4 translateTrans = glm::translate(glm::mat4(1.0f), glm::vec3(transX, transY, 0));
      glm::mat4 scaleTrans = glm::scale(glm::mat4(1.0f), glm::vec3(scaleX, scaleY, 1.0f));

      tileProj = scaleTrans * translateTrans * displayProj;
      //tileProj = translateTrans * displayProj;
      ...
      // Copy texture for display
      matToTexture(skinTexture, displaySkin);
      drawHand();
...
int main(int argc, char** argv)
{
  ...
  full_path = fs::system_complete( fs::path( argv[4] ) );
